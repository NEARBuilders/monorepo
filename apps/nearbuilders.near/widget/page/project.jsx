...
const { routes } = VM.require("${config/account}/widget/config.project") ?? {
  routes: {},
};

const { ProjectLayout } = VM.require(
  "${config/account}/widget/template.ProjectLayout"
) || {
  ProjectLayout: () => <></>,
};
  ...
  return (
    <div key={active}>
      <Widget
        src={src}
        props={{
          currentPath: `/${config/account}/widget/app?page=${page}&tab=${tab}`,
          page: tab,
          ...passProps,
          ...defaultProps,
        }}
