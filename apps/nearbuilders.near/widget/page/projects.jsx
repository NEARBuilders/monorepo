...
const { routes } = VM.require("${config/account}/widget/config.projects") ?? {
  routes: {},
};

const { theme } = VM.require("${config/account}/widget/config.theme") ?? {
  theme: {},
};

const { SidebarLayout } = VM.require(
  "${config/account}/widget/template.SidebarLayout"
) || {
  SidebarLayout: () => <></>,
};
